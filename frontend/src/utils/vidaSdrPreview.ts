...
export function buildSmartPreview(id: SmartPreviewId, context: number, data: VidaSdrDataLike, origens: string): SmartPreview {
  ...
  if (id === 'receita_recebida') return { title: 'Composição da receita gerada', description: 'Contratos que somam a receita deste agente — o que já entrou e o que ainda está previsto.', summary: [['Recebida', fmtBrl(data.receita_recebida || 0)], ['A receber', fmtBrl(data.receita_a_receber || 0)]], actionLabel: '', target: null, rows: [] }
  if (id === 'receita_a_receber') return { title: 'Valores a receber', description: 'Contratos com pagamentos ainda previstos (Financeiro + planilha de vendas).', summary: [['A receber', fmtBrl(data.receita_a_receber || 0)]], actionLabel: '', target: null, rows: [] }
